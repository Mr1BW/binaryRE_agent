...
_DEFAULT_TIMEOUT = int(os.getenv("DECOMPAI_SHELL_TIMEOUT", "120"))
...
class DockerizedBashProcess:
    ...
    def __init__(
        ...
        Args:
            ...
            timeout (int): Max seconds per command before termination (env: DECOMPAI_SHELL_TIMEOUT).
        """
    ...
    def _initialize_persistent_container(self, container_name: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"Started persistent container {container_name} with ID: {container_id}"
            )
            return container_id
    ...
    def run(
        self, commands: Union[str, List[str]], filter_color_codes: bool = True
    ) -> str:
        """
        ...
        if self.persistent:
            if not self.container_id:
                self.container_id = self._initialize_persistent_container(
                    self.container_name
                )
    ...
    def _run(self, command: str) -> str:
        ...
        try:
            ...
            output = (
                f"[TIMEOUT] Command exceeded {self.timeout}s limit and was terminated."
            )
    ...
    def _run_persistent(self, command: str) -> str:
        ...
        If the command exceeds the timeout, Ctrl+C is sent to interrupt it.
        """
        if (
            self.persistent_process is None
            or self.persistent_process.stdin is None
            or self.persistent_process.stdout is None
        ):
        ...
        fd = self.persistent_process.stdout.fileno()
        ...
        while True:
            ...
            if ready:
                ...
                if elapsed >= self.timeout:
                    self.persistent_process.stdin.write("\x03\n")
                    self.persistent_process.stdin.flush()
                    time.sleep(0.3)
                    try:
                        ready2, _, _ = select.select([fd], [], [], 0.5)
                        if ready2:
                            remaining_output = self.persistent_process.stdout.read(4096)
                            if remaining_output:
        ...
        if self.persistent_process.stderr:
            try:
                rlist, _, _ = select.select(
                    [self.persistent_process.stderr], [], [], 0.1
                )
                if rlist:
