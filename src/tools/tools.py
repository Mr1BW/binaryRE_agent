...
def create_tool_function(cls: Type) -> Callable:
    ...
    def dynamic_tool(state: Annotated[Any, InjectedState], **kwargs: Any) -> Any:
        session_path = state["session_path"]
        instance = cls(
            root_dir=os.path.join(session_path, settings.AGENT_WORKSPACE_NAME)
        )
...
def summarize_assembly(
    ...
    return ToolMessage(
        content=f"Summary of assembly code:\n\n{summary}", tool_call_id=tool_call_id
    )
...
def disassemble_binary(
    ...
    if (
        utils.count_tokens(assembly_code, state["model_name"])
        > state["model_context_length"] // 2
    ):
        raise ValueError("Disassembly too long for model context length.")

    return ToolMessage(
        content=f"Disassembly of binary:\n\n{assembly_code}", tool_call_id=tool_call_id
    )
...
def disassemble_section(
    ...
    return ToolMessage(
        content=f"Disassembly of section {section_name}:\n\n{assembly_code}",
...
def disassemble_function(
    ...
    return ToolMessage(
        content=f"Disassembly of function {function_name}:\n\n{assembly_code}",
        tool_call_id=tool_call_id,
    )
...
def get_string_at_address(
    ...
    address = int(address, 16)
    return ToolMessage(
        content=utils.get_string_at_address(state["binary_path"], address),
        tool_call_id=tool_call_id,
    )
...
class CustomSandboxedShellTool(SandboxedShellTool):
    args_schema: Type[BaseModel] = extend_args_schema(
        src_tools_sandboxed_shell_tool.SandboxedShellInput
    )
...
def run_ghidra_post_script(
    ...
    if not os.path.exists(full_script_path):
        return ToolMessage(
            content=f"Script not found at {script_path}", tool_call_id=tool_call_id
        )

    try:
        result = utils.run_ghidra_post_script(
            utils.get_binary_path_in_workspace(state["binary_path"]),
            full_script_path,
            script_args,
        )
        ...
        return ToolMessage(
            content=f"Error running Ghidra script: {str(e)}", tool_call_id=tool_call_id
        )
...
def decompile_function_with_ghidra(
    ...
    try:
        ...
        result = utils.decompile_function_with_ghidra(
            utils.get_binary_path_in_workspace(state["binary_path"]), function_name
        )
        return ToolMessage(content=result, tool_call_id=tool_call_id)
    except Exception as e:
        return ToolMessage(
            content=f"Error decompiling function: {str(e)}", tool_call_id=tool_call_id
        )
...
def r2_stateful_shell(
    ...
    new_lines = (
        output_lines[prev_line_count:] if prev_line_count < len(output_lines) else []
    )
...
def run_python_script(
    ...
    if script_name is None:
        ...
        while os.path.exists(
            os.path.join(workspace_path, f"script_{script_number}.py")
        ):
            script_number += 1
    ...
    try:
        ...
        return ToolMessage(
            content=f"Failed to write Python script: {str(e)}",
            tool_call_id=tool_call_id,
        )

    # Execute the script using the sandboxed shell
    try:
        result = kali_stateful_shell.invoke(
            {"commands": f"python {script_path}", "state": state}
        )
        return ToolMessage(
        ...
        return ToolMessage(
            content=f"{script_info}\n\nError running Python script: {str(e)}",
            tool_call_id=tool_call_id,
        )
...
def r2_write_patch(
    ...
    """
    Executes radare2 commands in WRITE mode (-w) to patch binary bytes.
    Use 'wx' to write hex bytes at current seek position.

    Common patches:
    - NOP out code: s <addr>; wx 9090909090
    - Flip conditional jump (JE 74 -> JNE 75): s <addr>; wx 75
    - Write raw bytes: wx <hex bytes>

    The binary path is automatically resolved. Use 'quit' to exit after commands.
    """
    if isinstance(commands, str):
        commands = [commands]
    full_command = f'r2 -w -qc "{"; ".join(commands)}" {utils.get_binary_path_in_workspace(state["binary_path"])}'
    return kali_stateful_shell.invoke({"commands": full_command, "state": state})
...
def patch_bytes(
    ...
    """
    Writes raw hex bytes at a file offset in the binary. Works on the workspace copy.

    Args:
        offset: File offset as hex string (e.g. '0x1050') or decimal string
        hex_bytes: Raw hex bytes to write (e.g. '909090' for 3 NOPs)
    """
    binary_path = utils.get_binary_path_in_workspace(state["binary_path"])
    try:
        offset_int = int(offset, 16) if offset.startswith("0x") else int(offset)
    except ValueError:
        return ToolMessage(
            content=f"Invalid offset: {offset}", tool_call_id=tool_call_id
        )
    try:
        data = bytes.fromhex(hex_bytes)
    except ValueError:
        return ToolMessage(
            content=f"Invalid hex bytes: {hex_bytes}", tool_call_id=tool_call_id
        )
    try:
        with open(binary_path, "r+b") as f:
            ...
            f.write(data)
        return ToolMessage(
            content=f"Patched {len(data)} byte(s) at offset {hex(offset_int)}: {hex_bytes}",
            tool_call_id=tool_call_id,
        )
    except Exception as e:
        return ToolMessage(content=f"Patch failed: {e}", tool_call_id=tool_call_id)
